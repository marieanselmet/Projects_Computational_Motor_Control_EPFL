...
def network_ode(_time, state, parameters):
    ...
    for i in range(len(phases)):
        ...
        for j in range(len(phases)):
            sum_couplings += amplitudes[j] * parameters.coupling_weights[i,j] * np.sin(phases[j]- phases[i]- parameters.phase_bias[i,j]) 
        dphases[i] = 2 * np.pi * parameters.freqs[i] + sum_couplings
...
def motor_output(phases, amplitudes, iteration=None):
    ...
    # limbs
    joint_angles[10:] = amplitudes[20:] * (1 + np.cos(phases[20:]))
