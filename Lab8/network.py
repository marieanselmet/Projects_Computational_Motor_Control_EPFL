...
def network_ode(_time, state, robot_parameters):
    ...
    """
    phases = state[:robot_parameters.n_oscillators]
    amplitudes = state[robot_parameters.n_oscillators:2*robot_parameters.n_oscillators]
    ...
    for i in range(len(phases)):
        ...
        for j in range(len(phases)):
            sum_couplings += amplitudes[j] * robot_parameters.coupling_weights[i,j] * np.sin(phases[j]- phases[i]- robot_parameters.phase_bias[i,j]) 
        dphases[i] = 2 * np.pi * robot_parameters.freqs[i] + sum_couplings
        damplitudes[i] = robot_parameters.rates[i] * (robot_parameters.nominal_amplitudes[i] - amplitudes[i])
...
def motor_output(phases, amplitudes, iteration=None):
    ...
    # Body
    joint_angles[:10] = amplitudes[:10] * (1 + np.cos(phases[:10])) - amplitudes[10:20] * (1 + np.cos(phases[10:20]))

    # Limbs
    joint_angles[10:] = phases[20:]
...
class SalamandraNetwork:
    ...
    def get_state(self, iteration):
        return self.state.phases(iteration)
